...
volatile uint16_t g_us_cap_size = IMAGE_BUFFER_SIZE;


/* Vsync signal information (true if it's triggered and false otherwise) */
static volatile uint32_t g_ul_vsync_flag = false;

volatile int captured_image_start = 0;
volatile int captured_image_end = 0;
...
/*configure image sensor pins */
	...
	ioport_set_pin_dir(OV_FSIN_GPIO,IOPORT_DIR_OUTPUT);
	//gpio_configure_pin(pin_camera_ret, PIO_OUTPUT_1);
	//ioport_set_pin_level(pin_camera_ret,LOW);
	//delay_ms(10);
	ioport_set_pin_level(OV_FSIN_GPIO,HIGH);
...
{
	 int ii, jj;
	for(ii=0;ii<IMAGE_BUFFER_SIZE-1;ii++){
		if(g_p_uc_cap_dest_buf[ii] == 0xff && g_p_uc_cap_dest_buf[ii+1] == 0xD8){
			break;
		}
	}
	if(ii==IMAGE_BUFFER_SIZE-1){
		return 0;
	}
	else{
		for(jj=0;(ii+jj)<IMAGE_BUFFER_SIZE-ii-1;jj++){
			if(g_p_uc_cap_dest_buf[ii+jj] == 0xff && g_p_uc_cap_dest_buf[ii+jj+1] == 0xD9){
				
				captured_image_start = ii;
				captured_image_end = ii+jj+2;
				return (jj+2);

			}
		}
			if(ii+jj==IMAGE_BUFFER_SIZE-1){
				return 0;
			}
	}
}
